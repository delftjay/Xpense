...
function navToggleInit()
{
	...
	//里程碑初始化
	$('.road-list dl').hover(function()
	{
		var windowWidth=$(window).width();
		if(windowWidth<1000)
			$(this).find('dd.font-gray').css('display','flex');
	},function()
	{
		var windowWidth=$(window).width();
		if(windowWidth<1000)
			$(this).find('dd.font-gray').css('display','none');
	});

	//architecture-content鼠标效果
	var windowWidth=$(window).width();
	if(windowWidth>500)
	{
		$('.architecture-content').hover(function()
		{
			$(this).css({
